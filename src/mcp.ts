...
export function createMcpServer(client = new DaemonClient()): McpServer {
  const server = new McpServer({ name: "grok-memory", version: "0.4.0" });
  ...
  server.registerTool("memory_remember", {
    ...
    inputSchema: { ...identityShape,
      ...
      mergeIntoMemoryId: z.string().uuid().optional().describe("Existing memory ID when this is the same lesson reached through a new situation; adds this trigger set without replacing or duplicating the thought."),
