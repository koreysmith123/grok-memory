...
test("HOK-001 HOK-002 HOK-003 before hook accepts future fields and emits only valid response JSON", async () => {
  const daemon = await fakeDaemon((path) => path === "/v1/recall" ? { additionalContext: "remembered context" } : { ok: true });
  try {
    ...
    assert.deepEqual(JSON.parse(output.stdout), { additional_context: "remembered context" }); assert.equal(output.stderr, "");
...
test("MCP-001 MCP-002 MCP-003 MCP-004 production MCP server initializes, lists, calls, and validates", async () => {
  ...
  try {
    ...
    assert.deepEqual(tools.tools.map((tool) => tool.name).sort(), ["memory_bind_bot", "memory_forget", "memory_health", "memory_inspect", "memory_rate", "memory_remember", "memory_search"]);
    const health = await client.callTool({ name: "memory_health", arguments: {} });
    assert.equal((health.structuredContent as any).ok, true);
    const invalid = await client.callTool({ name: "memory_search", arguments: { query: "missing identity" } });
...
test("EMU-001 EMU-002 EMU-004 emulator invokes production before/after hooks with Bot metadata", async () => {
  const daemon = await fakeDaemon((path) => path === "/v1/recall" ? { additionalContext: "private memory" } : { ok: true });
  ...
  try {
    ...
    assert.match(result.injected!, /private memory/); assert.deepEqual(daemon.requests.map((request) => request.path), ["/v1/recall", "/v1/turn"]);
    ...
    assert.equal(daemon.requests[1]?.body.input_tokens, 12); assert.equal(daemon.requests[1]?.body.output_tokens, 8);
