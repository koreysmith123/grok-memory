# RecallSmith — Release Requirements
...
| ID | Requirement | Measure / release gate |
|---|---|---|
| BOT-001 | The account-wide `RecallSmith` skill exposes the original NeoSmith three-level definitions, examples, resolution triggers, safety rules, and an idempotent current-Bot initialization flow. | Skill contract test verifies the preserved language and initialization contract; real GrokBot reports the skill enabled. |
...
| BOT-005 | Machine installation occurs once per shared GrokBot computer; later Bots perform only idempotent Bot-specific enrollment, preserving their existing descriptions and using no recurring scanner. | Skill and installer contracts distinguish healthy shared-service reuse from first installation; real profile inspection verifies preservation. |
...
| BOT-008 | A fresh Bot given only the repository URL and `Install this.` completes VM setup, native MCP registration, both skill installations, own-profile activation, health checks, and a disposable canary without questions or follow-up help. | Destructive clean-room GrokBot gate deletes prior Bots/data, creates one fresh Bot, sends one install message, and requires a self-reported `RECALLSMITH_READY` with independently inspected evidence. |
